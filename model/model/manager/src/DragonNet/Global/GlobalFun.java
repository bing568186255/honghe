...
public final class GlobalFun {
	...
	private static void zipHandle(ZipOutputStream out, OutputStream bo, File file) throws Exception {
	...
	public static void runbat(String batName) {
		try {
			...
			/*
			 * InputStream in = ps.getInputStream(); int c; while ((c =
			 * in.read()) != -1) { System.out.print(c);// 如果你不需要看输出，这行可以注销掉 }
			 * in.close();
			 */
			ps.waitFor();
		} catch (Exception ioe) {
	 ...
	 * @throws IOException
	 */
	public static List<HashMap> getRendomCoordinate(String addr) throws IOException {
		List<HashMap> list = new ArrayList<HashMap>();
		String[] x = getCoordinate(addr);
		String lngX = x[0];
		String latX = x[1];
		// 拼接医院,获取坐标
		try {
			String[] a = getCoordinate(addr + "医院");
			if (!StringUtils.isEmpty(a[0]) && !StringUtils.isEmpty(a[1])) {
				if (!a[0].equals(lngX) || !a[1].equals(latX)) {
					HashMap hash = new HashMap();
					hash.put("longitude", a[0]);
					hash.put("latitude", a[1]);
					list.add(hash);
				}
			}
		} catch (Exception e) {
		...
		// 拼接人民政府,获取坐标
		try {
			String[] a = getCoordinate(addr + "人民政府");
			if (!StringUtils.isEmpty(a[0]) && !StringUtils.isEmpty(a[1])) {
				if (!a[0].equals(lngX) || !a[1].equals(latX)) {
					HashMap hash = new HashMap();
					hash.put("longitude", a[0]);
		...
		// 拼接幼儿园,获取坐标
		try {
			String[] a = getCoordinate(addr + "幼儿园");
			if (!StringUtils.isEmpty(a[0]) && !StringUtils.isEmpty(a[1])) {
				if (!a[0].equals(lngX) || !a[1].equals(latX)) {
					HashMap hash = new HashMap();
					hash.put("longitude", a[0]);
		...
		// 拼接菜市场,获取坐标
		try {
			String[] a = getCoordinate(addr + "菜市场");
			if (!StringUtils.isEmpty(a[0]) && !StringUtils.isEmpty(a[1])) {
				if (!a[0].equals(lngX) || !a[1].equals(latX)) {
					HashMap hash = new HashMap();
					hash.put("longitude", a[0]);
		...
		// 拼接汽车站,获取坐标
		try {
			String[] a = getCoordinate(addr + "汽车站");
			if (!StringUtils.isEmpty(a[0]) && !StringUtils.isEmpty(a[1])) {
				if (!a[0].equals(lngX) || !a[1].equals(latX)) {
					HashMap hash = new HashMap();
					hash.put("longitude", a[0]);
		...
		// 拼接检察院,获取坐标
		try {
			String[] a = getCoordinate(addr + "检察院");
			if (!StringUtils.isEmpty(a[0]) && !StringUtils.isEmpty(a[1])) {
				if (!a[0].equals(lngX) || !a[1].equals(latX)) {
					HashMap hash = new HashMap();
					hash.put("longitude", a[0]);
		...
		// 拼接警察局,获取坐标
		try {
			String[] a = getCoordinate(addr + "警察局");
			if (!StringUtils.isEmpty(a[0]) && !StringUtils.isEmpty(a[1])) {
				if (!a[0].equals(lngX) || !a[1].equals(latX)) {
					HashMap hash = new HashMap();
					hash.put("longitude", a[0]);
		...
		// 拼接公园,获取坐标
		try {
			String[] a = getCoordinate(addr + "公园");
			if (!StringUtils.isEmpty(a[0]) && !StringUtils.isEmpty(a[1])) {
				if (!a[0].equals(lngX) || !a[1].equals(latX)) {
					HashMap hash = new HashMap();
					hash.put("longitude", a[0]);
		...
		// 拼接法院,获取坐标
		try {
			String[] a = getCoordinate(addr + "法院");
			if (!StringUtils.isEmpty(a[0]) && !StringUtils.isEmpty(a[1])) {
				if (!a[0].equals(lngX) || !a[1].equals(latX)) {
					HashMap hash = new HashMap();
					hash.put("longitude", a[0]);
		...
		// 拼接法院,获取坐标
		try {
			String[] a = getCoordinate(addr + "学校");
			if (!StringUtils.isEmpty(a[0]) && !StringUtils.isEmpty(a[1])) {
				if (!a[0].equals(lngX) || !a[1].equals(latX)) {
					HashMap hash = new HashMap();
					hash.put("longitude", a[0]);
